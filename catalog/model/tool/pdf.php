<?php
/*******************************************************************************
* Opencart PDF Generate Extension                                              *
*                                                                              *
* Version: 1.0                                                                 *
* Date:    2015-01-21                                                          *
* Author:  Joker Huang                                                         *
*                                                                              *
* This extension use FPDF and FPDF chinese unicode package                     *
...
class ModelToolPdf Extends Model
{
	...
	/**
	* create($title, $content, $is_download = true)
	* generate a pdf file
	*
	* @param string $title pdf title
	* @param string $content pdf main content
	* @param bool $is_download if true will download pdf file 
	*
	* @return 
	*/
	public function create($title, $content, $is_download = true)
	{
		try {
			$pdf = new PDF_Unicode($this->Big5_widths, $this->GB_widths, $this->config->get('pdf_header'), $this->config->get('pdf_footer'));
			$pdf->AddGBFont();
			$pdf->AddUniCNShwFont('uni'); 
			$pdf->AddPage();
			
			// set title
			$pdf->setFont('uni', 'B', 30);
			$title = str_replace(array("\n", "\r", "\t", "\v", "\f"), '', $title);
			$pdf->Cell(0, 10, $title, 0, 1, 'C');
			
			// print a new null string
			$pdf->ln(10);
			...
			if ($is_download) {
				$pdf->Output(date('Y_m_d_H_i_s') . rand(111, 999) . '.pdf', 'D');
			} else {
				$pdf->Output();
			}
...
/*******************************************************************************
* FPDF Chinese Extension                                                       *
*******************************************************************************/
class PDF_Chinese extends FPDF
{
public function __construct($big5, $gb)
{
	$this->Big5_widths = $big5;
	$this->GB_widths = $gb;
}
...
class PDF_Unicode extends PDF_Chinese 
{
	...
	public $pdf_header;  // pdf header set
	public $pdf_footer;  // pdf footer set

	function __construct($big5, $gb, $pdf_header = '', $pdf_footer = '', $charset = 'UTF-8')
	{
		parent::__construct($big5, $gb);
	...
	public function Header()
	{
		if ($this->pdf_header) {
			...
			$this->Cell(0, 0, $this->pdf_header, 0, 1, 'L');
			$this->Line(0, 13, 300, 13);
			$this->Ln(5);
		}
	}
	...
	public function Footer()
	{
		if ($this->pdf_footer) {
			...
			$this->setFont('uni', '', 9);
			$this->SetTextColor(136, 136, 136);
			$this->Cell(0, 0, $this->pdf_footer, 0, 0, 'R');
		}
	}
